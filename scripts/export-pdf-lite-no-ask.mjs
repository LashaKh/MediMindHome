...
const OUT_PATH = '/Users/toko/Desktop/Pitch PDF/MediMind-Pitch-no-ask-lite.pdf';
// JPEG quality for the embedded slide images (DCTDecode). The reference
// "lite" PDF used JPEG at this rough quality to land ~5MB at 2560×1440.
...
async function main() {
  ...
  log('Removing the two Ask slides (12 THE ASK + 13 WHAT YOU\'RE INVESTING IN)…');
  await page.evaluate(() => {
    // Slide 12 = section.askx-section. Slide 13 = the bare <section> that wraps
    // the .terms-layout ("How your $200K becomes equity"). Remove both, then
    // ask Reveal to re-index so getTotalSlides()/Reveal.slide(i) skip them.
    const removed = [];
    const ask = document.querySelector('section.askx-section');
    if (ask) { ask.remove(); removed.push('askx-section'); }
    const terms = document.querySelector('.terms-layout');
    const termsSection = terms && terms.closest('.reveal .slides > section');
    if (termsSection) { termsSection.remove(); removed.push('terms-section'); }
    window.Reveal.sync();
    return removed;
  }).then((removed) => log(`  removed: ${removed.join(', ') || 'NONE — selectors missed!'}`));
...
# argv layout: [pngs..., quality, tour_idx, walkthrough_url, out]
out = sys.argv[-1]
...
# 1) PNGs → multipage PDF via Pillow, JPEG-compressed (DCTDecode). Passing
#    quality makes Pillow embed each page as a JPEG instead of lossless Flate,
#    which is what keeps the "lite" PDF ~5MB at 2560×1440 (vs ~14MB for PNG).
imgs = [Image.open(p).convert('RGB') for p in pngs]
imgs[0].save(out, 'PDF', save_all=True, append_images=imgs[1:],
             resolution=192.0, quality=quality)
...
`;
  const merge = spawnSync(
    venvPython,
    ['-c', pyScript, ...pngFiles, String(JPEG_QUALITY), String(tourSlideIndex), WALKTHROUGH_URL, OUT_PATH],
    { stdio: ['ignore', 'pipe', 'pipe'] }
  );
