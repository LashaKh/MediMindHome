...
-- Create view with explicit column references and table aliases
...
BEGIN
  ...
  -- Insert the note with explicit user ID
  ...
  RETURNING patient_notes.id INTO v_note_id;

  -- Return the complete note data from the view with explicit column references
  RETURN QUERY
  SELECT 
    pnw.note_id as id,
    pnw.patient_id,
