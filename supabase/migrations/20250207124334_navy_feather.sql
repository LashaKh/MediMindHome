...
DROP POLICY IF EXISTS "Enable note read" ON patient_notes;
DROP POLICY IF EXISTS "Enable note create" ON patient_notes;
...
TO authenticated
USING (
  -- Allow update if user created the note OR has access to the patient
...
TO authenticated
USING (
  -- Allow delete if user created the note OR has access to the patient
  created_by = auth.uid() OR
  EXISTS (
    SELECT 1 FROM patients
    WHERE id = patient_notes.patient_id
